...
function Attendance() {
  ...
  const visibleRows = initialAttendance.filter((record) => {
    if (showOriginal) return record.monthKey === '2026-06'
    const monthMatch = record.monthKey === month
    const employeeMatch = employee === 'All Employees' || record.name === employee
  ...
  return (
    <div className="page-stack">
      <div className="stats-grid three">
        <StatCard icon={FiCheckCircle} label="Present" value={visibleRows.filter((row) => row.status === 'Present').length} meta="Selected month data" tone="green" />
        <StatCard icon={FiClock} label="Late" value={visibleRows.filter((row) => row.status === 'Late').length} meta="After grace period" tone="amber" />
        <StatCard icon={FiCalendar} label="Leave/Absent" value={visibleRows.filter((row) => ['Leave', 'Absent'].includes(row.status)).length} meta="Leave and absent count" tone="blue" />
      </div>
      ...
      <div className="grid-two">
        ...
        <Panel title={showOriginal ? 'Original Attendance Data' : 'Filtered Attendance Data'}>
          <DataTable
            columns={['Employee', 'Date', 'In', 'Out', 'Hours', 'Status']}
            rows={visibleRows}
            renderRow={(record) => (
...
function Payroll({ employees }) {
  ...
  return (
    <div className="page-stack">
      ...
      <Panel title="Payroll Register" action={<button className="primary-button"><FiDownload /> Generate Payslips</button>}>
        <DataTable
          columns={['Employee', 'Department', 'Basic', 'HRA', 'Allowance', 'Deductions', 'Net Salary']}
          rows={payroll}
          renderRow={(row) => (
            <tr key={row.employee}>
...
function Documents() {
  return (
    <div className="page-stack">
      <Panel title="HR Document Center" action={<button className="primary-button"><FiPlus /> Create Document</button>}>
        <DataTable
          columns={['Document', 'Owner', 'Category', 'Updated', 'Status']}
          rows={documents}
          renderRow={(document) => (
...
function EmployeePayslip({ employee }) {
  ...
  return (
    <div className="page-stack">
      ...
      <Panel title="My Payslip" action={<button className="primary-button"><FiDownload /> Download PDF</button>}>
        <DataTable
          ...
          rows={[
            ...
            ['PF Deduction', `-${money(pf)}`],
            ['Tax Deduction', `-${money(tax)}`],
            ['Net Salary', money(net)],
          ]}
    ...
    <div className="app-shell">
      ...
      <main className="content">
        <Topbar
          ...
          onNewRequest={() => setShowRequest(true)}
